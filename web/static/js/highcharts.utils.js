...
function loadReleaseDevelPieChart(container, title, data){
    ...
    let options = {
        ...
        colors: ['#c12e2a', '#F6AE2D', '#4F1271', '#B8E2C8', '#8e1919', '#878787', '#540000', '#d9534f', '#808080'],
         //move 1 position right the gray #878787 if new release appear TODO
...
async function loadCudaDonutChartPerReleaseDetail(chartId, preparedList, cuda_chart_URL, title) {
    ...
    releasesToShowInverted.forEach(branchName => {
        ...
        branchData.forEach(item => {
            if (!item.cuda || item.cuda.toLowerCase() === "unknown") return;
